...
import 'package:test_pkp/data/datasource/postdatasource/default_post_data_source.dart';
import 'package:test_pkp/data/datasource/postdatasource/post_data_source.dart';
import 'package:test_pkp/data/datasource/userdatasource/default_user_data_source.dart';
import 'package:test_pkp/data/repository/default_post_repository.dart';
import 'package:test_pkp/data/repository/default_user_repository.dart';
import 'package:test_pkp/domain/entity/post/create_post_parameter.dart';
import 'package:test_pkp/domain/entity/post/create_post_response.dart';
import 'package:test_pkp/domain/entity/post/delete_post_parameter.dart';
import 'package:test_pkp/domain/entity/post/delete_post_response.dart';
import 'package:test_pkp/domain/entity/post/edit_post_parameter.dart';
import 'package:test_pkp/domain/entity/post/edit_post_response.dart';
import 'package:test_pkp/domain/entity/post/get_post_comment_parameter.dart';
import 'package:test_pkp/domain/entity/post/get_post_comment_response.dart';
import 'package:test_pkp/domain/entity/post/get_post_list_parameter.dart';
import 'package:test_pkp/domain/entity/post/get_post_list_response.dart';
import 'package:test_pkp/domain/entity/post/get_post_parameter.dart';
import 'package:test_pkp/domain/entity/post/get_post_response.dart';
import 'package:test_pkp/domain/entity/post/post.dart';
import 'package:test_pkp/domain/entity/post/post_comment.dart';
import 'package:test_pkp/domain/repository/post_repository.dart';
import 'package:test_pkp/domain/usecase/create_post.dart';
import 'package:test_pkp/domain/usecase/delete_post.dart';
import 'package:test_pkp/domain/usecase/edit_post.dart';
import 'package:test_pkp/domain/usecase/get_post.dart';
import 'package:test_pkp/domain/usecase/get_post_comment.dart';
import 'package:test_pkp/domain/usecase/get_post_list.dart';
import 'package:test_pkp/misc/http_client.dart';
import 'package:test_pkp/misc/load_data_result.dart';

void main() {
  group('Post Use Case Test', () {
    late PostDataSource postDataSource;
    late PostRepository postRepository;

    // Create post
    late CreatePost createPost;
    late CreatePostParameter createPostParameter;

    // Delete post
    late DeletePost deletePost;
    late DeletePostParameter deletePostParameter;

    // Edit post
    late EditPost editPost;
    late EditPostParameter editPostParameter;

    // Get post
    late GetPost getPost;
    late GetPostParameter getPostParameter;

    // Get post comment
    late GetPostComment getPostComment;
    late GetPostCommentParameter getPostCommentParameter;

    // Get post list
    late GetPostList getPostList;
    late GetPostListParameter getPostListParameter;
    ...
    setUp(() {
      ...
      postDataSource = DefaultPostDataSource(dio: dio);
      postRepository = DefaultPostRepository(postDataSource: postDataSource);

      // Create post
      createPost = CreatePost(postRepository: postRepository);
      createPostParameter = CreatePostParameter(userId: "1", title: "Test", body: "Test");

      // Delete post
      deletePost = DeletePost(postRepository: postRepository);
      deletePostParameter = DeletePostParameter(postId: '1');

      // Edit post
      editPost = EditPost(postRepository: postRepository);
      editPostParameter = EditPostParameter(postId: "1", userId: "1", title: "Test", body: "Test");

      // Get post
      getPost = GetPost(postRepository: postRepository);
      getPostParameter = GetPostParameter(postId: "1");

      // Get post comment
      getPostComment = GetPostComment(postRepository: postRepository);
      getPostCommentParameter = GetPostCommentParameter(postId: "1");

      // Get post list
      getPostList = GetPostList(postRepository: postRepository);
      getPostListParameter = GetPostListParameter();
    });
    ...
    test('create post', () async {
      ...
      LoadDataResult<CreatePostResponse> createPostResponseLoadDataResult = await createPost.execute(createPostParameter);
      expect(createPostResponseLoadDataResult is SuccessLoadDataResult<CreatePostResponse>, true);
    });

    test('delete post', () async {
      dioAdapter.onDelete(
        "/posts/${deletePostParameter.postId}",
        (server) => server.reply(
          200,
          jsonDecode("{\"success\":true}")
        ),
      ...
      LoadDataResult<DeletePostResponse> deletePostResponseLoadDataResult = await deletePost.execute(deletePostParameter);
      expect(deletePostResponseLoadDataResult is SuccessLoadDataResult<DeletePostResponse>, true);
    });

    test('edit post', () async {
      dioAdapter.onPut(
        "/posts/${editPostParameter.postId}",
        (server) => server.reply(
          200,
          jsonDecode("{\"success\":true}")
        ),
      ...
      LoadDataResult<EditPostResponse> editPostResponseLoadDataResult = await editPost.execute(editPostParameter);
      expect(editPostResponseLoadDataResult is SuccessLoadDataResult<EditPostResponse>, true);
    });
    ...
    test('get post', () async {
      ...
      LoadDataResult<GetPostResponse> getPostResponseLoadDataResult = await getPost.execute(getPostParameter);
      expect(getPostResponseLoadDataResult is SuccessLoadDataResult<GetPostResponse>, true);
      Post post = (getPostResponseLoadDataResult as SuccessLoadDataResult<GetPostResponse>).value.post;
      expect(post.userId == "1" && post.id == "1", true);
    });

    test('get post comments', () async {
      dioAdapter.onGet(
        "/posts/${getPostParameter.postId}/comments",
        (server) => server.reply(
          200,
          jsonDecode("[{\"postId\":1,\"id\":1,\"name\":\"id labore ex et quam laborum\",\"email\":\"Eliseo@gardner.biz\",\"body\":\"laudantium enim quasi est quidem magnam voluptate ipsam eos\\ntempora quo necessitatibus\\ndolor quam autem quasi\\nreiciendis et nam sapiente accusantium\"},{\"postId\":1,\"id\":2,\"name\":\"quo vero reiciendis velit similique earum\",\"email\":\"Jayne_Kuhic@sydney.com\",\"body\":\"est natus enim nihil est dolore omnis voluptatem numquam\\net omnis occaecati quod ullam at\\nvoluptatem error expedita pariatur\\nnihil sint nostrum voluptatem reiciendis et\"},{\"postId\":1,\"id\":3,\"name\":\"odio adipisci rerum aut animi\",\"email\":\"Nikita@garfield.biz\",\"body\":\"quia molestiae reprehenderit quasi aspernatur\\naut expedita occaecati aliquam eveniet laudantium\\nomnis quibusdam delectus saepe quia accusamus maiores nam est\\ncum et ducimus et vero voluptates excepturi deleniti ratione\"},{\"postId\":1,\"id\":4,\"name\":\"alias odio sit\",\"email\":\"Lew@alysha.tv\",\"body\":\"non et atque\\noccaecati deserunt quas accusantium unde odit nobis qui voluptatem\\nquia voluptas consequuntur itaque dolor\\net qui rerum deleniti ut occaecati\"},{\"postId\":1,\"id\":5,\"name\":\"vero eaque aliquid doloribus et culpa\",\"email\":\"Hayden@althea.biz\",\"body\":\"harum non quasi et ratione\\ntempore iure ex voluptates in ratione\\nharum architecto fugit inventore cupiditate\\nvoluptates magni quo et\"}]")
        ),
        queryParameters: {},
        headers: {},
      );
      LoadDataResult<GetPostCommentResponse> getPostCommentResponseLoadDataResult = await getPostComment.execute(getPostCommentParameter);
      expect(getPostCommentResponseLoadDataResult is SuccessLoadDataResult<GetPostCommentResponse>, true);
      List<PostComment> postCommentList = (getPostCommentResponseLoadDataResult as SuccessLoadDataResult<GetPostCommentResponse>).value.postComment;
      expect(postCommentList.length, 5);
    });
  });
}
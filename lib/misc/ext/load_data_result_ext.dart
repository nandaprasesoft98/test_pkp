...
extension LoadDataResultExt<T> on LoadDataResult<T> {
  ...
  LoadDataResult<O> map<O>(MapLoadDataResultType<O, T> onMap) {
    if (this is SuccessLoadDataResult<T>) {
      ...
      return FailedLoadDataResult<O>(e: (this as FailedLoadDataResult<T>).e);
    } else {
      return FailedLoadDataResult<O>(e: LoadDataResultError(message: "Load data result is not suitable."));
    }
  }
}